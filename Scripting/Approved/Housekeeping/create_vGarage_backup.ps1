...
#$Drive = "C:"
$RequiredSpaceGB = 33
$AVL ="$Drive\ITM\Backup\vGarage\AVL"
$ITK ="$Drive\ITM\Backup\vGarage\ITK"
$TT ="$Drive\ITM\Backup\vGarage\TT"
$SYN ="$Drive\ITM\Backup\vGarage\SYN"
$BEG ="$Drive\ITM\Backup\vGarage\BEG"
...
New-Item -Path "$Drive\ITM" -Name "Backup" -ItemType "Directory" -Force
##Ordner vGarage erstellen
New-Item -Path "$Drive\ITM\Backup" -Name "vGarage" -ItemType "Directory" -Force

##Vererbung Deaktivieren
$acl = Get-Acl "$Drive\ITM\Backup\vGarage"
$acl.SetAccessRuleProtection($true,$true)
$acl | Set-Acl "$Drive\ITM\Backup\vGarage"

##Authentifizierten Benuzter entfernen
icacls.exe "$Drive\ITM\Backup\vGarage" --%  /remove[:g] *S-1-5-11
...
foreach ($Folder in $Folder) {
    $newpath = Join-Path "$Drive\ITM\Backup\vGarage" -ChildPath $Folder
...
icacls.exe $Drive\ITM\Backup\vGarage /restore 'C:\Windows\System32\SiL-Housekeeping\cre.txt' /T
...
icacls.exe "$Drive\ITM\Backup\vGarage" --%  /grant *S-1-5-11:(REA,RD,X,RA)
